...
export const Screens = {
    ...
    GAME_MAZE: 'gameMaze', GAME_WORD: 'gameWord', GAME_CHECKERS: 'gameCheckers',
    GAME_TIC_TAC: 'gameTicTac', GAME_MEMORY: 'gameMemory', GAME_BATTLESHIP: 'gameBattleship',
    GAME_SUDOKU: 'gameSudoku', GAME_SHOOT: 'gameShoot', GAME_COMPARE: 'gameCompare',
    GAME_FINGER: 'gameFinger', GAME_COMPOSITION: 'gameComposition',
    GAME_CLOCK: 'gameClock', GAME_CHANGE: 'gameChange',
    PET_MODULE: 'petModule',
    SHOP: 'shop', INVENTORY: 'inventory',
    CHESTS: 'chests', ACHIEVEMENTS: 'achievements',
    DAILY_TASKS: 'dailyTasks', REFERENCE: 'reference',
...
export function navigateTo(screen, params = {}) {
    ...
    const handlers = {
        ...
        [Screens.ENCYCLOPEDIA]: () => import('../modules/encyclopedia.js').then(m => m.renderEncyclopedia(app)),
        [Screens.REFERENCE]: () => import('../modules/encyclopedia.js').then(m => m.renderEncyclopedia(app)),
    ...
    if (handler) { handler(); }
    else { import('./ui.js').then(m => m.renderMainMenu(app)); }
    
    if (AppState.currentChild) { saveState(AppState.currentChild.id); }
    console.log(`📍 ${screen}`);
}
